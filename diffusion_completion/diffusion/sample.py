...
# -*- coding: utf-8 -*-
"""
Inference & Evaluation Script for Conditional DDPM on 2D LiDAR Completion

Provides:
- DiffusionSampler: runs reverse diffusion loop
- DiffusionModelWrapper: wraps DDPM with same interface as baseline model
- evaluate_diffusion: full evaluation pipeline
- CLI for inference with optional visualization
"""
...
class DiffusionModelWrapper(nn.Module):
    ...
    This enables reuse of baseline evaluation (ModelEvaluator) and
    visualization (visualize_samples) code.

    Usage:
        wrapper = DiffusionModelWrapper(model, scheduler, device)
...
def evaluate_diffusion(
    ...
    Args:
        model: ConditionalDDPMUNet
        scheduler: NoiseScheduler
        data_loader: DataLoader with (partial, mask, target) batches
        device: Torch device

    Returns:
        dict with keys: n_samples, summary, per_sample, predictions
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Inference & Evaluation for Conditional DDPM"
    )
    ...
    parser.add_argument("--save_per_sample", action="store_true",
                        help="Save per-sample metrics to per_sample_diffusion.json")
    ...
    result_file = output_dir / "evaluation_diffusion.json"
    with open(result_file, 'w') as f:
        json.dump(save_results, f, indent=2)
    print(f"\nResults saved to: {result_file}")

    if args.save_per_sample:
        per_sample_file = output_dir / "per_sample_diffusion.json"
